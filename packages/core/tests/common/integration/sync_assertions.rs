...
pub async fn assert_ledger_parity_for_base(
  ...
  let storages_a = get_storages_for_base(client, url_a, token_a, base_id).await;
  let storages_b = get_storages_for_base(client, url_b, token_b, base_id).await;
  ...
  assert_eq!(
    sa, sb,
    "{label_a} and {label_b} disagree on storages for base {base_id}"
  );

  let entries_a = get_all_ledger_entries(client, url_a, token_a).await;
  let entries_b = get_all_ledger_entries(client, url_b, token_b).await;
  ...
  assert_eq!(
    ...
    "ledger entries differ for base {base_id} between {label_a} and {label_b}"
  );
}
