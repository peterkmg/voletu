...
#[tokio::test]
async fn target_matching_is_delimiter_safe_for_base_ids() {
...
#[tokio::test]
async fn outbound_returns_push_payload_shape_with_json_strings() {
  let db = Arc::new(setup_db().await);
  let service = sync_service_with_node(db.clone(), TEST_SYNC_NODE_ID);
...
#[tokio::test]
async fn audit_log_query_applies_offset_and_limit_in_ascending_id_order() {
  let db = Arc::new(setup_db().await);
  let service = sync_service_with_node(db.clone(), TEST_SYNC_NODE_ID);
...
#[tokio::test]
async fn empty_scope_advances_to_highest_evaluated_id() {
  with_audit_context(Uuid::now_v7(), Uuid::now_v7(), || async {
    let db = Arc::new(setup_db().await);
    let service = sync_service_with_node(db.clone(), TEST_SYNC_NODE_ID);
...
#[tokio::test]
async fn returns_global_and_targeted_logs_for_requesting_base_scope() {
  with_audit_context(Uuid::now_v7(), Uuid::now_v7(), || async {
    let db = Arc::new(setup_db().await);
    let service = sync_service_with_node(db.clone(), TEST_SYNC_NODE_ID);
...
#[tokio::test]
async fn excludes_roles_and_local_tables_from_scope() {
  with_audit_context(Uuid::now_v7(), Uuid::now_v7(), || async {
    let db = Arc::new(setup_db().await);
    let service = sync_service_with_node(db.clone(), TEST_SYNC_NODE_ID);
    let catalog = seed_inventory_catalog(&db).await;
