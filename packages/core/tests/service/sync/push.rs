...
#[tokio::test]
async fn rejects_lower_role_update_when_newer_higher_role_log_exists() {
...
#[tokio::test]
async fn restores_company_from_snapshot_and_is_idempotent_on_reapply() {
  with_audit_context(Uuid::now_v7(), Uuid::now_v7(), || async {
    let db = Arc::new(setup_db().await);
    let service = sync_service_with_node(db.clone(), TEST_SYNC_NODE_ID);
...
#[tokio::test]
async fn skips_duplicate_log_ids_within_same_batch() {
  with_audit_context(Uuid::now_v7(), Uuid::now_v7(), || async {
    let db = Arc::new(setup_db().await);
    let service = sync_service_with_node(db.clone(), TEST_SYNC_NODE_ID);
...
#[tokio::test]
async fn watermark_upsert_updates_existing_row_for_same_node_and_direction() {
  let db = Arc::new(setup_db().await);
  let service = sync_service_with_node(db.clone(), TEST_SYNC_NODE_ID);
  let catalog = seed_inventory_catalog(&db).await;
...
#[tokio::test]
async fn status_reflects_local_topology_and_local_row_keeps_central_url() {
  with_audit_context(Uuid::now_v7(), Uuid::now_v7(), || async {
    let db = Arc::new(setup_db().await);
    let catalog = seed_inventory_catalog(&db).await;
