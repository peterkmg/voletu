...
#[tokio::test]
async fn endpoints_filter_by_document_number_and_status() {
...
#[tokio::test]
async fn blending_status_filter_returns_posted_only_and_rejects_invalid_status() {
  let (db, app, token) = setup_seeded_app_with_admin_token().await;
  let ctx = seed_inventory_context(&db).await;
...
#[tokio::test]
async fn dispatch_and_reconciliation_reject_invalid_status_values() {
...
#[tokio::test]
async fn acceptance_and_ownership_reject_invalid_status_values() {
  let (_db, app, token) = setup_seeded_app_with_admin_token().await;
