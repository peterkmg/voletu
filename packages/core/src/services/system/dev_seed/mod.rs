...
use super::{local::load_local_bootstrap, SystemService};
use crate::{
  api::ApiError,
...
use documents::{seed_inventory_documents, seed_transport_documents};
use helpers::{
  company_name,
  fake_fragment,
  location_name,
...
pub(super) struct ReferencePool {
  product_type_ids: Vec<Uuid>,
  product_group_ids: Vec<Uuid>,
  product_ids: Vec<Uuid>,
  component_product_ids: Vec<Uuid>,
  target_product_ids: Vec<Uuid>,
...
pub(super) struct TransportPool {
  truck: Vec<WaybillSeed>,
  rail: Vec<WaybillSeed>,
}

#[derive(Clone, Debug, Default)]
pub(super) struct DocumentCounts {
  acceptance_docs: usize,
  dispatch_docs: usize,
  blending_docs: usize,
  ownership_transfers: usize,
  physical_transfers: usize,
    ...
    for group_index in 0..rng.random_range(2..=3) {
      ...
      for product_index in 0..rng.random_range(2..=4) {
        ...
        let saved = product::ActiveModel {
          ...
          add_identification: Set(Some(ctx.tag.label().to_string())),
