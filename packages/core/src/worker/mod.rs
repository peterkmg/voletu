  ...
  tokio::spawn(async move {
    ...
    loop {
      tokio::select! {
        ...
        _ = tick.tick() => {
          // Run the tick body in a labeled block so every exit path flows
          // to the single `bump_tick_count()` call at the bottom. This way
          // the tick counter reliably reflects "the worker had a chance to
          // check for work, regardless of what it decided to do". The
          // `/sync/await-cycle` endpoint uses this to distinguish "worker
          // is stuck" from "worker checked and there was nothing to do".
          'tick_body: {
            ...
            // Local status fetch: we only use `highest_audit_log_id` here
            ...
            // Scope-aware status probe: include our base assignments so
            // Central can compute a `highest_matching_id` that only
            // advances when data relevant to OUR scope exists.
            ...
            // Check if Central has new IN-SCOPE data we haven't pulled
            // yet. We compare `highest_matching_id` (scope-aware), not
            // `highest_audit_log_id`, against our current PULL cursor.
            // When the stored discriminant no longer matches the current
            // one, treat the cursor as nil — any matching id on Central
            // counts as new work.
          ...
          // Single bump point for the whole tick. Any path through the
          // tick body — including early-outs (non-peripheral, offline,
          // caught up) and errors — increments this counter. The
          // `/sync/await-cycle` endpoint waits for this to advance past
          // the value it captured at entry, which proves the worker has
          // actually had a chance to evaluate new work.
