...
export function AcceptanceTable({ data }: AcceptanceTableProps) {
  ...
  const columns = useMemo(() => getAcceptanceColumns(t), [t])
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const source = String(row.original.sourceEntity ?? '').toLowerCase()
      const search = String(filterValue).toLowerCase()
      return docNumber.includes(search) || source.includes(search)
    },
