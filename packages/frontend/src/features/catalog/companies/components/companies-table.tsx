...
export function CompaniesTable({ data }: CompaniesTableProps) {
  ...
  const columns = useMemo(() => getCompanyColumns(t), [t])
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const legal = String(row.original.legalName ?? '').toLowerCase()
      const search = String(filterValue).toLowerCase()
      return name.includes(search) || legal.includes(search)
    },
  ...
  const bulkActions: BulkAction<CompanyResponse>[] = [
    {
